...
- `min_selection`
- `max_selection` nullable
- `is_active`
- timestamps

Seed hệ thống gồm `WASH_PACKAGE` (`single`, min/max `1/1`) và `ADD_ON` (`multiple`, min `0`, max null).
Policy thuộc group trong database, client/admin service form không được tự gửi min/max/mode.
...
- `name`
- `description`
- `service_group_id` bắt buộc, FK tới `service_groups`
- `is_active`
- timestamps
...
`capacity_units_override` chỉ dùng khi service thật sự cần nhiều sức chứa vật lý hơn vehicle default; nó không
biểu diễn độ phức tạp hay thời gian. Catalog hiện tại để override của Standard, Premium, Tire Care và Engine
Clean là null. Premium không làm phương tiện lớn hơn; thời gian xử lý dài hơn nằm ở `duration_minutes`.
...
- Standard và Premium thuộc `WASH_PACKAGE`, thay thế lẫn nhau; mỗi booking mới phải có đúng một package.
- Tire Care và Engine Clean thuộc `ADD_ON`, không được đặt nếu thiếu package.
- UI render radio cho group `single`, checkbox cho `multiple`; backend luôn kiểm tra lại policy từ database.
...
- Selection policy phải pass trước khi tính subtotal/duration/capacity hoặc áp dụng benefit. Không tự thêm
  Standard khi chọn Premium; booking item chỉ chứa service thực sự được chọn.
...
- Request Standard + Premium, add-on-only hoặc không có package bị từ chối trước mọi booking/item/reservation/
  research/audit write. Duplicate service ID vẫn bị chặn khi chuẩn hóa request.
- Booking lịch sử, kể cả record từng chứa cả Standard và Premium, tiếp tục đọc theo service ID và snapshot cũ;
  không reprice, recalculate capacity hay revalidate.
