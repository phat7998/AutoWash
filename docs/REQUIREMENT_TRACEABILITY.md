...
| REP-02 | Admin aggregate; revenue completed-only; admin-only; trang báo cáo riêng có date range an toàn | Spec §7.10 | MUST | Report / 14 + UX correction | `DashboardController`, `AdminReportController`, `DashboardService`, `AdminReportService`, `ResearchReportRepository`, admin dashboard/report views | `AdminReportDateRangeValidatorTest`, `ResearchFlowTest`, `AuthAuthorizationTest` | DEMO-08 | Done |
