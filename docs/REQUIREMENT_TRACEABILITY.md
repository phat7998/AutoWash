...
| CAT-01 | Chỉ service/cặp giá active, supported, đúng type; giá từ DB | Spec §7.3 | MUST | Catalog / 06 | `app/Controllers/CatalogController.php`, `app/Services/ServiceCatalogService.php`, `app/Repositories/ServiceCatalogRepository.php` | `CatalogSlotFlowTest`, `ServiceCatalogValidatorTest` | DEMO-02 | Done |
| CAT-02 | Price/duration/capacity override theo unique service + vehicle type và snapshot | Spec §7.3; DEC-016 | MUST | Pricing / 02,06,07 | `app/Repositories/BookingRepository.php`, `app/Services/BookingResourceCalculator.php`, `app/Services/BookingService.php` | `DatabaseFoundationTest`, `CatalogSlotFlowTest`, `BookingRulesTest`, `BookingFlowTest` | DEMO-02 | Done |
...
| BKG-03 | Server price; một perk/promo/reward; final ≥0 | Spec §7.4 | MUST | Booking+Pricing / 07,12 | `app/Services/PriceCalculator.php`, `app/Services/BookingService.php`, `app/Repositories/BookingRepository.php` | `BookingRulesTest`, `BookingFlowTest`; perk/promo/reward tiếp tục Slice 12 | DEMO-07 | In Progress |
...
| BKG-05 | Customer hủy ≥2h; admin ngoại lệ; giải phóng units, không earn/penalty | Spec §7.4; DEC-021 | MUST | Booking / 08,12 | `app/Services/BookingLifecyclePolicy.php`, `app/Services/BookingService.php`, `app/Controllers/BookingController.php`, `app/Controllers/AdminBookingController.php` | `BookingLifecyclePolicyTest`, `BookingLifecycleFlowTest`; reward restore tiếp tục Slice 12 | DEMO-04 | In Progress |
| BKG-06 | Complete atomic và idempotent: metrics, loyalty, usages, event | Spec §7.4 | MUST | Booking+Loyalty / 08,09,12,14 | `app/Contracts/BookingCompletionProcessorInterface.php`, `app/Services/BookingService.php`, `app/Repositories/BookingRepository.php` | `BookingLifecycleFlowTest`; loyalty/usage/event tiếp tục Slice 09/12/14 | DEMO-04 | In Progress |
| BKG-07 | Multi-service duration = sum; capacity = max; giữ trên mọi slot overlap; backend source | Spec §7.4; DEC-017 | MUST | Booking / 07 | `app/Services/BookingResourceCalculator.php`, `app/Services/BookingService.php`, `app/Repositories/BookingRepository.php` | `BookingRulesTest`, `BookingFlowTest` (BKG-CL-01..07) | DEMO-03 | Done |
...
| LOY-02 | Ledger + generic credit/debit allocation + balance cache atomic; không âm/clamp | Spec §7.5; DEC-011/018/032 | MUST | Loyalty / 09,10 | `app/Services/LoyaltyService.php`, `app/Services/LoyaltyDebitAllocator.php`, `app/Repositories/LoyaltyTransactionRepository.php`, migration 007, reconcile CLI | `LoyaltyFlowTest`, `RewardFlowTest`, `LoyaltyMigrationBackfillTest` | DEMO-04/05 | Done |
...
| RWD-04 | Ownership và use-once | Spec §7.7 | MUST | Reward / 10,12 | Owner-only list/redemption ở Reward module; checkout use-once tiếp tục Slice 12 | `RewardFlowTest`; use-once checkout tiếp tục Slice 12 | DEMO-05 | In Progress |
...
| ADM-02 | Service CRUD/inactive; bảo toàn price snapshot | Spec §7.9 | MUST | Admin Catalog / 06 | `app/Controllers/AdminServiceController.php`, `app/Services/ServiceCatalogService.php`, `resources/views/admin/services/` | `CatalogSlotFlowTest` | DEMO-04 | Done |
