...
| DEC-011 | Ledger là nguồn lịch sử; `users.point_balance` chỉ là cache/tổng hợp. | Mọi mutation qua LoyaltyService và reconcile được. | LOY-02 |
...
| DEC-018 | Duyệt `loyalty_allocations`. | Mỗi debit redeem/expire truy vết earn lot, points và allocated_at; unique debit+earn. | LOY-02..04 |
...
| DEC-032 | Adjustment âm bị từ chối nếu vượt available points; không clamp về 0. | Reason bắt buộc; mọi adjust có ledger/audit, có thể tham chiếu source transaction; transaction/lock bảo đảm `available_points + adjustment_points >= 0`. | LOY-02, ADM-06, NFR-21/22 |
