        ...
        <?php foreach ($vehicleTypes as $type): ?>
            ...
            <section class="price-row" aria-labelledby="type-<?= $e($key) ?>">
                <div class="price-row-heading">
                    ...
                    <label class="check-field"><input type="checkbox" name="prices[<?= $e($key) ?>][is_active]" value="1" <?= $active ? 'checked' : '' ?>> Cấu hình active</label>
                </div>
                <div class="form-grid form-grid-three">
                    ...
                    <div class="form-field"><label for="capacity-<?= $e($key) ?>">Capacity override</label><input id="capacity-<?= $e($key) ?>" name="prices[<?= $e($key) ?>][capacity_units_override]" inputmode="numeric" value="<?= $e($price['capacity_units_override'] ?? '') ?>" aria-invalid="<?= isset($errors['prices.' . $key . '.capacity_units_override']) ? 'true' : 'false' ?>"><span class="field-help">Để trống để dùng mặc định của loại xe.</span><?php if (isset($errors['prices.' . $key . '.capacity_units_override'])): ?><span class="field-error"><?= $e($errors['prices.' . $key . '.capacity_units_override']) ?></span><?php endif; ?></div>
                </div>
            </section>
        <?php endforeach; ?>
    </fieldset>
